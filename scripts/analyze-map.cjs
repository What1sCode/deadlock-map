...
async function main() {
  ...
  console.log('\nTop 25 component sizes:');
  sorted.slice(0, 25).forEach((c, i) => console.log(`  #${i+1}: ${c.size} px  (label ${c.label})`));
  ...
  console.log('\nDistribution:');
  console.log(`  <50px:       ${buckets.tiny}`);
  console.log(`  50-500px:    ${buckets.small}`);
  console.log(`  500-5000px:  ${buckets.medium}`);
  console.log(`  5000px+:     ${buckets.large}`);
}
